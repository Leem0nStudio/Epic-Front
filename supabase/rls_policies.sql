    ...
    -- Allow authenticated users to insert their own player data
    ...
    -- Allow authenticated users to insert their own units
    CREATE POLICY "Allow authenticated players insert own units" ON units
        FOR INSERT TO authenticated
        WITH CHECK (auth.uid() = player_id);

    -- Allow authenticated users to update their own units
    ...
    -- Allow authenticated users to insert their own inventory
    CREATE POLICY "Allow authenticated players insert inventory" ON inventory
        FOR INSERT TO authenticated
        WITH CHECK (auth.uid() = player_id);

    -- Allow authenticated users to update their own inventory
    ...
    -- Allow authenticated users to insert their own party
    CREATE POLICY "Allow authenticated players insert party" ON party
        FOR INSERT TO authenticated
        WITH CHECK (auth.uid() = player_id);

    -- Allow authenticated users to update their own party
    ...
    -- Allow authenticated users to insert their own recruitment queue entries
    CREATE POLICY "Allow authenticated players insert recruitment_queue" ON recruitment_queue
        FOR INSERT TO authenticated
        WITH CHECK (auth.uid() = player_id);

    -- Allow authenticated users to update their own recruitment queue entries
    ...
    -- Allow authenticated users to insert their own gacha state
    CREATE POLICY "Allow authenticated players insert gacha_state" ON gacha_state
        FOR INSERT TO authenticated
        WITH CHECK (auth.uid() = player_id);

    -- Allow authenticated users to update their own gacha state
    ...
    -- Allow authenticated users to insert their own campaign progress
    CREATE POLICY "Allow authenticated players insert campaign_progress" ON campaign_progress
        FOR INSERT TO authenticated
        WITH CHECK (auth.uid() = player_id);

    -- Allow authenticated users to update their own campaign progress
