-- Drop existing to start clean as requested ("diseñar desde cero")
...
CREATE TABLE game_configs (
    ...
    config_data JSONB, -- Optional catch-all
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 2. Data-Driven Tables (Versioned)
CREATE TABLE jobs (
    id TEXT NOT NULL,
    version TEXT NOT NULL REFERENCES game_configs(version),
    name TEXT NOT NULL,
...
CREATE TABLE inventory (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    player_id UUID REFERENCES players(id) ON DELETE CASCADE,
    item_id TEXT NOT NULL, -- References jobs/skills/cards/weapons table 'id'
    item_type TEXT NOT NULL, -- 'card', 'weapon', 'skill_scroll', 'material'
...
CREATE POLICY "Players can view/edit own profile" ON players FOR ALL USING (auth.uid() = id);

ALTER TABLE gacha_state ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Gacha state is private" ON gacha_state FOR ALL USING (auth.uid() = player_id);

ALTER TABLE units ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Units are private" ON units FOR ALL USING (auth.uid() = player_id);

ALTER TABLE inventory ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Inventory is private" ON inventory FOR ALL USING (auth.uid() = player_id);

ALTER TABLE party ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Party is private" ON party FOR ALL USING (auth.uid() = player_id);

ALTER TABLE recruitment_queue ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Recruitment is private" ON recruitment_queue FOR ALL USING (auth.uid() = player_id);

-- Static tables read-only for public/auth
ALTER TABLE game_configs ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Everyone can read configs" ON game_configs FOR SELECT USING (true);
ALTER TABLE jobs ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Everyone can read jobs" ON jobs FOR SELECT USING (true);
ALTER TABLE skills ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Everyone can read skills" ON skills FOR SELECT USING (true);
ALTER TABLE cards ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Everyone can read cards" ON cards FOR SELECT USING (true);
ALTER TABLE weapons ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Everyone can read weapons" ON weapons FOR SELECT USING (true);
...
-- 2. Pull Gacha (Updated for components)
...
DECLARE
    ...
    v_target_id TEXT;
    v_target_name TEXT;
    v_target_type TEXT; -- 'card', 'weapon', 'skill', 'job_core'
BEGIN
    -- Determine cost
    ...
    -- Multi discount (multi 10 = 9 pulls price)
    IF p_amount >= 10 THEN
        v_total_cost := (p_amount - 1) * v_cost_per_pull;
    ELSE
        v_total_cost := p_amount * v_cost_per_pull;
    END IF;
    ...
    FOR i IN 1..p_amount LOOP
        ...
        -- Special case: Job Core in Legendary
        IF v_rarity = 'legendary' AND random() < 0.1 THEN
            v_target_type := 'job_core';
        END IF;

        -- Fetch random item of that rarity/type
        -- Note: simplified logic here, normally you'd UNION or use a master item table
        -- For this demo we'll prioritize cards and weapons
...
BEGIN
    ...
    IF v_current_job_id IS DISTINCT FROM v_parent_job_id THEN
        RAISE EXCEPTION 'Incorrect evolution path';
    END IF;
    ...
    IF v_materials IS NOT NULL AND jsonb_array_length(v_materials) > 0 THEN
        FOR v_material IN SELECT * FROM jsonb_array_elements(v_materials) LOOP
            ...
            WHERE player_id = v_user_id
              AND item_id = v_material->>'itemId'
              AND quantity >= (v_material->>'amount')::INTEGER;

            IF NOT FOUND THEN RAISE EXCEPTION 'Missing materials'; END IF;
        END LOOP;
