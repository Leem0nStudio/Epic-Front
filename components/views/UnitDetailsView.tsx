...
export function UnitDetailsView({ unitId, onNavigate, onUpdate, onOpenInventory }: UnitDetailsViewProps) {
  ...
  const handleEvolve = async (jobId: string) => {
    try {
      ...
      alert(e.message || "Error al evolucionar");
    }
  };
    ...
    <div className="flex flex-col h-full bg-[#020508] overflow-hidden relative font-sans">
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-8 pb-12">
        ...
        <div className="relative">
            ...
            <div className="relative bg-white/5 border border-white/10 rounded-[40px] p-8 overflow-hidden group">
                ...
                <div className="flex items-center justify-center py-6 relative">
                    ...
                    <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(245,199,107,0.1),transparent)] rounded-full animate-pulse" />
                </div>

                {/* Combat Stats Mini Grid */}
                <div className="grid grid-cols-3 gap-3 mt-6">
                   <StatBox label="ATK" value={finalStats.atk} icon={<Sword size={10} />} color="text-red-400" />
                   <StatBox label="DEF" value={finalStats.def} icon={<Shield size={10} />} color="text-blue-400" />
                   <StatBox label="AGI" value={finalStats.agi} icon={<Activity size={10} />} color="text-green-400" />
                   <StatBox label="HP" value={finalStats.hp} icon={<Zap size={10} />} color="text-cyan-400" />
                   <StatBox label="MATK" value={finalStats.matk} icon={<Sparkles size={10} />} color="text-purple-400" />
                   <StatBox label="MDEF" value={finalStats.mdef} icon={<Shield size={10} />} color="text-indigo-400" />
                </div>
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4">
            <h3 className="text-[10px] font-black text-white/40 uppercase tracking-[0.3em] px-2 flex items-center gap-2">
        ...
        {evolutions.length > 0 && (
            <div className="space-y-4">
                <h3 className="text-[10px] font-black text-[#F5C76B] uppercase tracking-[0.3em] px-2 flex items-center gap-2">
...
function EquipSlot({ label, item, onOpen, icon, mini = false }: any) {
    return (
        <button
            onClick={onOpen}
            className={`relative bg-white/5 border border-white/10 rounded-3xl flex items-center transition-all hover:bg-white/10 ${mini ? 'flex-col gap-2 p-3 text-center' : 'p-4 gap-4'}`}
        >
            <div className={`${mini ? 'w-10 h-10' : 'w-12 h-12'} rounded-2xl bg-black/40 border border-white/5 flex items-center justify-center text-white/20`}>
                {item ? <Sparkles size={mini ? 16 : 20} className="text-[#F5C76B]" /> : icon}
            </div>
